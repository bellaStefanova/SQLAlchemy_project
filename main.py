...
from decorators import session_starter
from models import Recipe, Base, Chef
...
create_recipe('Spaghetti Carbonara', 'Pasta, Eggs, Pancetta, Cheese',
              'Cook the pasta, mix it with eggs, pancetta, and cheese')
create_recipe('Chicken Stir-Fry', 'Chicken, Bell Peppers, Soy Sauce, Vegetables',
              'Stir-fry chicken and vegetables with soy sauce')
create_recipe('Caesar Salad', 'Romaine Lettuce, Croutons, Caesar Dressing',
...
update_recipe_by_name(name="Spaghetti Carbonara",
...
def get_recipes_by_ingredient(ingredient_name: str):
    recipes = session.query(Recipe).filter(Recipe.ingredients.like(f'%{ingredient_name}%')).all()
    return recipes
...
recipe1 = create_recipe('Spaghetti Bolognese', 'Ground beef, tomatoes, pasta',
                        'Cook beef, add tomatoes, serve over pasta')
recipe2 = create_recipe('Chicken Alfredo', 'Chicken, fettuccine, Alfredo sauce',
                        'Cook chicken, boil pasta, mix with sauce')
recipe3 = create_recipe('Chicken Noodle Soup', 'Chicken, noodles, carrots', 'Boil chicken, add noodles, carrots')
...
def get_recipes_with_chef():
    recipes = session.query(Recipe.name, Chef.name.label('chef_name')).join(
        Chef, Recipe.chef).all()
    return '\n'.join(f'Recipe: {recipe_name} made by chef: {chef_name}' for
                     recipe_name, chef_name in recipes)
...
recipe1 = Recipe(name="Beef Wellington",
                 ingredients="Beef fillet, Puff pastry, Mushrooms, Foie gras",
                 instructions="Prepare the fillet and encase it in puff pastry.")

recipe1.chef = chef1

recipe2 = Recipe(name="Boeuf Bourguignon",
                 ingredients="Beef, Red wine, Onions, Carrots",
                 instructions="Slow-cook the beef with red wine and vegetables.")

recipe2.chef = chef2

recipe3 = Recipe(name="Spaghetti Carbonara",
                 ingredients="Spaghetti, Eggs, Pancetta, Cheese",
                 instructions="Cook pasta, mix ingredients.")

recipe3.chef = chef3

recipe4 = Recipe(name="Chocolate Cake",
                 ingredients="Chocolate, Flour, Sugar, Eggs",
                 instructions="Bake a delicious chocolate cake.")

recipe4.chef = chef4

recipe5 = Recipe(name="Chicken Tikka Masala",
